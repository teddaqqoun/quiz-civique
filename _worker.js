      ...
      if (url.pathname.startsWith('/api/admin/sandbox/') && request.method === 'POST') {
        try {
          ...
          if (url.pathname === '/api/admin/sandbox/bug-report') {
            ...
            await env.DB.prepare('INSERT INTO bug_reports (page_url, description) VALUES (?, ?)').bind(body.page_url || null, body.description).run();
            return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } });
          }
          if (url.pathname === '/api/admin/sandbox/question-report') {
            await env.DB.prepare('INSERT INTO question_reports (question_text, level, theme, reason) VALUES (?, ?, ?, ?)').bind(body.question_text || null, body.level || null, body.theme || null, body.reason || null).run();
            return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } });
          }
        } catch (err) {
